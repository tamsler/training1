...
function putUserV1(req, res, next) {
    ...
    mongo.update("users", {'_id': updateUser._id}, updateUser, function(err, numUpdatedDocs) {
...
function postUserLoginV1(req, res, next) {
    ...
    console.log("DEBUG: postUserLoginV1 : user : ", user);

    mongo.findOne("users", { 'loginId': user.loginId, 'password' : user.password }, {}, function(err, users) {

            if(err) {

                res.send(500, "ERROR: postUserLoginV1 : find");
                return next();
            }
            else if(users) {
